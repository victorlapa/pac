...
import { Thermometer, Droplets, CloudSun, Power } from "lucide-react"
...
export function ControladorArmazenamentoCafeComponent() {
  ...
  useEffect(() => {
    ...
    const intervalo = setInterval(() => {
      ...
      setUmidade(prev => Math.max(30, Math.min(70, prev + (Math.random() - 0.5) * 2)))

      // Simula mudanças de temperatura externa
      setTempExterna(prev => Math.max(15, Math.min(35, prev + (Math.random() - 0.5))))
    }, 5000)
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      <h1 className="text-3xl font-bold text-center mb-8">Controlador de Armazenamento de Grãos de Café</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Temperatura de Armazenamento</CardTitle>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Temperatura Externa</CardTitle>
          ...
          <div className="h-[200px]">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={historicoTemp}>
                    ...
                    if (active && payload && payload.length) {
                      return (
                        <div className="bg-background p-2 border rounded shadow">
                          <p className="text-foreground">Hora: {payload[0].payload.hora}</p>
                          <p className="text-foreground">Temperatura: {payload[0].value}°C</p>
                        </div>
                      );
                    }
                    return null;
                ...
                />
                <Line type="monotone" dataKey="temperatura" stroke="hsl(var(--primary))" strokeWidth={2} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </CardContent>
